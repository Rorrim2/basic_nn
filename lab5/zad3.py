...
class CNN:
    def __init__(self, input_shape, filters_shape, filters_count, output_count, alpha):
        ...
        self.second_layer = Layer(self.random_array(output_count, int((neurons_count_x*neurons_count_y * filters_count)/4),
                                                    -0.1, 0.1))
    ...
    def predict(self, input_vector):
        ...
        kernel_layer_relu = self.relu(kernel_layer)

        kernel_layer_relu = self.max_pooling(kernel_layer_relu, 2, 2)
        kernel_layer_flatten = np.reshape(kernel_layer_relu,
                                          (1, kernel_layer_relu.shape[0] * kernel_layer_relu.shape[1]))
    ...
    def fit(self, input_vector, expected_output):
        ...
        kernel_layer_relu_pool = self.max_pooling(kernel_layer_relu, 2, 2)
        kernel_layer_flatten = np.reshape(kernel_layer_relu_pool, (1, kernel_layer_relu_pool.shape[0] * kernel_layer_relu_pool.shape[1]))
        kernel_layer_flatten = kernel_layer_flatten[0]
        layer_2_values = np.matmul(kernel_layer_flatten, np.transpose(self.second_layer.layer_weights))
        ...
        print(kernel_layer_relu_pool.shape)
        print(layer_1_delta.shape)

        layer_1_delta_reshaped = np.reshape(layer_1_delta, kernel_layer_relu_pool.shape)
    ...
    def max_pooling(self, image, mask_size, step):
